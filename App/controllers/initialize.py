...
from .recipe import create_recipe, create_r_ingredient
...
def initialize():
    ...
    for category in category_list:
        create_category(category['idCategory'], category['strCategory'], category['strCategoryThumb'])
    ...
    for recipe_detail in recipe_list:
        create_recipe(recipe_detail['idMeal'], recipe_detail['strMeal'], recipe_detail['strInstructions'], recipe_detail['strMealThumb'], recipe_detail['strCategory'])
        string = "strIngredient"
        i = 1

        while recipe_detail[f'{string + str(i)}'] != None and recipe_detail[f'{string + str(i)}'] != "":
            create_r_ingredient(recipe_detail['idMeal'], recipe_detail[f'{string + str(i)}'])
            i = i + 1
            if i == 21:
                break
